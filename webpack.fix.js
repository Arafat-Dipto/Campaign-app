...
const hmrOptions = {
	...
	host: isHotWithSSL ? host : process.platform === "win32" ? "127.0.0.1" : "0.0.0.0",
	port: "8080",
};
	...
	if (isHotWithSSL) {
		...
		devServer = {
			https: {
				...
				key: fs.readFileSync(path.resolve(process.env.HOME, `.config/valet/Certificates/${host}.key`)),
				cert: fs.readFileSync(path.resolve(process.env.HOME, `.config/valet/Certificates/${host}.crt`)),
			},
		};
	}
}
